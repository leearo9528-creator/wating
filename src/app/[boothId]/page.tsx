...
import { BoothInfoCard } from "@/components/booth-info-card";
...
export default function UserWaitingPage() {
  ...
  const handleCancel = useCallback(async () => {
    if (myWaitlistId) await cancelWaitlist(myWaitlistId);
  ...
  const boothName = boothInfo?.name || "로딩중...";
  const boothDescription = boothInfo?.description || "";
  const isOperating = !boothInfo?.status || boothInfo.status === 'open';

  if (viewState === 'register') {
    return (
      <main className="min-h-dvh bg-secondary">
        <BoothHeader boothName={boothName} location={boothDescription} />

        <div className="mx-auto flex max-w-md flex-col gap-3 px-5 pt-2 pb-4">
          <BoothInfoCard
            name={boothName}
            description={boothInfo?.description}
            photoUrl={boothInfo?.photo_url}
          />
  ...
  return (
    <main className="min-h-dvh bg-secondary pb-24">
      ...
      <ActionBar onCancel={handleCancel} />
    </main>
  );
}
